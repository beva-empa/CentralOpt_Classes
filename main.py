...
if __name__ == '__main__':
    ...
    for hub in range(1, num_hubs + 1):
        ...
        list_techs.append(capacities_conv.loc[((capacities_conv.hub == hub) & (capacities_conv.value > 0)),
                                         'tech'].values.tolist())
        list_storage.append(capacities_stor.loc[((capacities_stor.hub == hub) & (capacities_stor.value > 0)),
                                           'tech'].values.tolist())
    ...
    if 'Battery' in list_storage:
        Battery_max = (capacities_stor.loc[((capacities_stor.hub == hub) &
                                            (capacities_stor.tech == 'Battery')), 'value'].squeeze())
    else:
        Battery_max = 0

    if 'heat_storage' in list_storage:
        Thermal_max = (capacities_stor.loc[((capacities_stor.hub == hub) &
                                            (capacities_stor.tech == 'heat_storage')), 'value'].squeeze())
    else:
        Thermal_max = 0

    elec_storage = 0.25 * Battery_max
    thermal_storage = 0.25 * Thermal_max

    CHP_Runtime = 0
    CHP_Downtime = 12

    battery_depth = [0, 0, 0, 0]
    if Battery_max > 0:
        for j in range(4):
            bat_SOC = elec_storage / Battery_max
            if bat_SOC >= ((j + 1) * 0.2):
                battery_depth[j] = 0.2

            elif bat_SOC < ((j + 1) * 0.2) and (bat_SOC >= 0.2 * j):

                battery_depth[j] = bat_SOC - 0.2 * j
    ...
    while (i < 3) and (init == False):
        ...
        (thermal_storage_end, elec_storage_end,
         battery_depth_end, CHP_ontime, CHP_offtime) = main_optimize(elec_storage, battery_depth, thermal_storage,
        ...
        if (thermal_storage - 0.1 * Thermal_max) <= thermal_storage_end <= (thermal_storage + 0.1 * Thermal_max):
            init = True
    ...
    while time_now <= time_end:
        ...
        (demand_power, demand_heat, power, heat, grid_in, grid_out, storage_heatin, storage_heatout,
         storage_elecin, storage_elecout, tech_cost, storage_cost, grid_cost,
         final_cost, storage_heat, storage_elec, batt_depth, CHP_ontime,
         CHP_offtime) = main_optimize(elec_storage, battery_depth, thermal_storage, CHP_Runtime, CHP_Downtime,
                                      capacities_conv, capacities_stor, tech_details, storage_details, list_techs,
                                      list_storage, time_now)
        ...
        if Battery_max > 0:
            temp = storage_elec * 100 / Battery_max
            SOC_elec.append([temp])
        else:
            SOC_elec.append(0)

        if Thermal_max > 0:
            temp1 = storage_heat * 100 / Thermal_max
            SOC_therm.append([temp1])
        else:
            SOC_therm.append(0)
    ...
    with xlsxwriter.Workbook('results.xlsx') as workbook:
        ...
        saveChp(workbook, CHP_ontime_final, CHP_offtime_final, list_techs, date_info, time_info)
